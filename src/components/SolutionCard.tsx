...
export const SolutionCard = ({
  ...
  const content = <Card className={`group relative overflow-hidden transition-all duration-500 h-full ${disabled ? 'opacity-40 cursor-not-allowed bg-gray-50/50' : 'cursor-pointer bg-white/80 backdrop-blur-sm hover:bg-white/95 hover:-translate-y-2 hover:scale-[1.02] hover:border-primary/30'}`} style={{
    ...
    if (!disabled) {
      ...
      <div className="p-8 flex flex-col items-center text-center space-y-5 h-full">
        ...
        <div className="space-y-1.5 flex-1 flex flex-col justify-start">
          <h3 className="font-semibold text-base text-foreground leading-tight min-h-[2.5rem]">{title}</h3>
          {region && <p className={`text-lg tracking-wide ${disabled ? 'text-gray-700' : 'text-muted-foreground'}`}>
              {region}
            </p>}
          {disabled}
        </div>
